...
const StatsPanel: React.FC<StatsPanelProps> = ({
  ...
  <div>
    <div className="mb-4">
      ...
      <div
        className="goal-section p-3 rounded-3"
        style={{
      ...
      >
        ...
        <small className={isDarkMode ? 'text-light' : 'text-dark'}>
          本章字數: {wordCount}/{dailyGoal} 字 ({Math.round((wordCount / dailyGoal) * 100)}%)
        </small>
      </div>
    </div>
    <div className="mb-4">
      ...
      <div className="d-flex flex-column gap-3">
        <div
          className="stat-card p-3 rounded-3"
          style={{ background: 'linear-gradient(45deg, #4a90e2, #357abd)', border: 'none' }}
        >
        ...
        <div
          className="stat-card p-3 rounded-3"
          style={{ background: 'linear-gradient(45deg, #00b894, #00a085)', border: 'none' }}
        >
          <div className="d-flex justify-content-between align-items-center text-white">
            <span className="fw-bold">
        ...
        <div
          className="stat-card p-3 rounded-3"
          style={{ background: 'linear-gradient(45deg, #fd79a8, #e84393)', border: 'none' }}
        >
          <div className="d-flex justify-content-between align-items-center text-white">
            <span className="fw-bold">
            ...
            <span className="fs-6 fw-bold">
              {Math.ceil(chapters.reduce((sum, ch) => sum + ch.wordCount, 0) / 250)}分鐘
            </span>
          </div>
        </div>
      </div>
