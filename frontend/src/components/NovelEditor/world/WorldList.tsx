...
const WorldList: React.FC<Props> = ({ novelId, isDarkMode }) => {
  ...
  useEffect(() => {
    worldApi
      ...
      .catch((err) => setError(describeError(err, '載入世界觀失敗。')));
  }, [novelId]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(describeError(err, '儲存世界觀項目失敗。'));
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setError(describeError(err, '刪除世界觀項目失敗。'));
    }
  };
    ...
    <div>
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h6 className={`mb-0 fw-bold ${isDarkMode ? 'text-white' : 'text-dark'}`}>
          ...
          <span
            ...
            style={{ background: 'rgba(0,184,148,0.25)', color: '#55efc4', fontSize: '0.7rem' }}
          >
            {items.length}
          </span>
        </h6>
        <button
          style={pillBtn('rgba(39,174,96,0.2)', '#6fcf97')}
          onMouseEnter={(e) => {
      ...
      <div className="d-flex flex-wrap gap-1 mb-3">
        <button
          style={{
            ...pillBtn(
              filter === 'all' ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.07)',
              isDarkMode ? '#fff' : '#333',
            ),
            border: filter === 'all' ? '1px solid rgba(255,255,255,0.3)' : '1px solid transparent',
          }}
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              ...
              style={{
                ...pillBtn(
                  active ? `${cat.color}33` : 'rgba(255,255,255,0.07)',
                  active ? cat.color : isDarkMode ? 'rgba(255,255,255,0.5)' : '#666',
                ),
      ...
      {showForm && (
        <div
          ...
          style={{
            background: isDarkMode ? 'rgba(0,184,148,0.1)' : 'rgba(0,184,148,0.06)',
        ...
        >
          ...
          <div className="d-flex gap-2">
            <button
              style={pillBtn('rgba(74,144,226,0.3)', '#a0c4f1')}
              onMouseEnter={(e) => {
              ...
              onMouseLeave={(e) => {
                ...
                e.currentTarget.style.color = '#a0c4f1';
              }}
            ...
            <button
              style={pillBtn('rgba(255,255,255,0.1)', isDarkMode ? '#aaa' : '#666')}
            ...
            >
              ...
              >
                ...
                <div className="flex-grow-1 min-width-0">
                  ...
                  {item.description && !isExpanded && (
                    <div
                      className={`small mt-1 text-truncate ${isDarkMode ? 'text-light opacity-60' : 'text-muted'}`}
                ...
                <i
                  className={`bi ${isExpanded ? 'bi-chevron-up' : 'bi-chevron-down'} ${isDarkMode ? 'text-light opacity-50' : 'text-muted'}`}
              ...
              {isExpanded && (
                ...
                >
                  {item.description && (
                    <div
                      className={`small pt-3 ${isDarkMode ? 'text-light' : 'text-dark'}`}
                  ...
                  <div className="d-flex gap-2 mt-3">
                    <button
                      style={pillBtn('rgba(74,144,226,0.2)', '#a0c4f1')}
                      onMouseEnter={(e) => {
                        e.currentTarget.style.background = 'rgba(74,144,226,0.45)';
                        e.currentTarget.style.color = '#fff';
                      }}
                      onMouseLeave={(e) => {
                    ...
                    <button
                      style={pillBtn('rgba(231,76,60,0.18)', '#ff8a80')}
                      onMouseEnter={(e) => {
