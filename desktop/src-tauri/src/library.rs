...
impl LibraryRepository {
    ...
    pub fn list_works(&self) -> Result<Vec<WorkSummary>, LibraryError> {
        ...
        let works = statement.query_map([], |row| {
            Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?, row.get::<_, String>(2)?, row.get::<_, String>(3)?))
        })?.collect::<Result<Vec<_>, _>>()?;

        works.into_iter().map(|(id, title, status, updated_at)| {
            let chapters = Self::chapters_for(&connection, &id)?;
            Ok(WorkSummary { id, title, status, chapters, updated_at })
        }).collect()
    }

    pub fn create_work(&self, requested_title: Option<&str>) -> Result<WorkSummary, LibraryError> {
        let title = requested_title.map(str::trim).filter(|value| !value.is_empty()).unwrap_or("未命名作品");
        ...
        transaction.commit()?;
        Ok(WorkSummary {
            id: work_id.clone(), title: title.to_owned(), status: "concept".into(), updated_at: now,
            chapters: vec![ChapterSummary { id: chapter_id, work_id, title: "第一章".into(), sort_order: 0, word_count: 0 }],
        })
    }
    ...
    pub fn load_chapter(&self, chapter_id: &str) -> Result<ChapterDocument, LibraryError> {
        ...
        connection.query_row(
            ...
            [chapter_id],
            |row| Ok(ChapterDocument { chapter_id: chapter_id.to_owned(), schema_version: row.get(0)?, text: row.get(1)?, saved_at: row.get(2)? }),
    ...
    pub fn save_chapter(&self, chapter_id: &str, text: &str) -> Result<ChapterDocument, LibraryError> {
        let now = Utc::now().to_rfc3339_opts(SecondsFormat::Millis, true);
        let content = json!({ "type": "doc", "schemaVersion": 1, "content": [{ "type": "paragraph", "content": [{ "type": "text", "text": text }] }] }).to_string();
        let word_count = text.chars().filter(|character| !character.is_whitespace()).count() as i64;
        let mut connection = self.connection.lock().expect("library mutex poisoned");
        let transaction = connection.transaction()?;
        ...
        if changed == 0 { return Err(LibraryError::ChapterNotFound); }
        transaction.execute("UPDATE chapters SET word_count = ?1, updated_at = ?2 WHERE id = ?3", params![word_count, now, chapter_id])?;
        transaction.execute("UPDATE works SET updated_at = ?1 WHERE id = (SELECT work_id FROM chapters WHERE id = ?2)", params![now, chapter_id])?;
        transaction.commit()?;
        Ok(ChapterDocument { chapter_id: chapter_id.to_owned(), schema_version: 1, text: text.to_owned(), saved_at: now })
    }

    fn chapters_for(connection: &Connection, work_id: &str) -> Result<Vec<ChapterSummary>, rusqlite::Error> {
        ...
        statement.query_map([work_id], |row| Ok(ChapterSummary { id: row.get(0)?, work_id: work_id.to_owned(), title: row.get(1)?, sort_order: row.get(2)?, word_count: row.get(3)? }))?.collect()
    }
}
...
mod tests {
    ...
    fn creating_a_work_atomically_creates_the_first_chapter_and_document() {
        ...
        assert_eq!(repository.load_chapter(&work.chapters[0].id).unwrap().text, "");
    }
    ...
    fn saving_updates_the_authoritative_document_projection() {
        ...
        assert_eq!(repository.load_chapter(chapter_id).unwrap().text, "第一句。 ");
        assert_eq!(repository.list_works().unwrap()[0].chapters[0].word_count, 4);
    }
}
