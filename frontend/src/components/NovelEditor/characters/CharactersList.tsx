...
const CharactersList: React.FC<Props> = ({ novelId, isDarkMode }) => {
  ...
  useEffect(() => {
    charactersApi
      ...
      .catch((err) => setError(describeError(err, '載入角色失敗。')));
  }, [novelId]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(describeError(err, '儲存角色失敗。'));
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setError(describeError(err, '刪除角色失敗。'));
    }
  };
    ...
    <div>
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h6 className={`mb-0 fw-bold ${isDarkMode ? 'text-white' : 'text-dark'}`}>
          ...
          <span
            ...
            style={{ background: 'rgba(231,76,60,0.25)', color: '#ff8a80', fontSize: '0.7rem' }}
          >
            {characters.length}
          </span>
        </h6>
      ...
      {showForm && (
        <div
          ...
          style={{
            background: isDarkMode ? 'rgba(231,76,60,0.12)' : 'rgba(231,76,60,0.06)',
        ...
        >
          ...
          <Form.Control
            size="sm"
            type="url"
            placeholder="頭像圖片網址（選填，http/https）"
                ...
                >
                  <div className="pt-3 d-flex flex-column gap-2">
                    {c.description && (
                      <div>
                        <div
                          className={`small fw-bold mb-1 ${isDarkMode ? 'text-light opacity-60' : 'text-muted'}`}
                          style={{
                            fontSize: '0.7rem',
                        ...
                        <div
                          className={`small ${isDarkMode ? 'text-light' : 'text-dark'}`}
      ...
      {characters.length === 0 && !showForm && (
        ...
        >
          <i
            className="bi bi-person-plus d-block mb-2"
