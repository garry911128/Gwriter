...
const AIAssistant: React.FC<AIAssistantProps> = ({
  ...
  const handleSuggest = async (type: AISuggestType) => {
    ...
    try {
      ...
      // 後端會回具體原因（例如「章節內容為空」），不要一律換成同一句通用訊息。
      setError(describeError(err, '無法連線至 AI 服務，請確認 Ollama 已啟動且模型已下載。'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="d-flex flex-wrap gap-2 mb-3">
        {SUGGEST_TYPES.map(({ type, label, icon }) => (
          <Button
            key={type}
            variant={
              activeType === type && (suggestion || titleOptions.length > 0)
                ? 'light'
                : isDarkMode
                  ? 'outline-light'
                  : 'outline-dark'
